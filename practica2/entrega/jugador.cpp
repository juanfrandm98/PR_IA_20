...
Action ComportamientoJugador::think( Sensores sensores ) {
	...
	if( !hayplan ) {
		...
		cout << "VOY A CALCULAR" << endl;
		hayplan = pathFinding( sensores.nivel, actual, destino, plan );
		cout << "HE CALCULADO" << endl;
		objetivo = DESTINO;
	}
	...
	if( sensores.nivel == 4 ) {
		...
		if( conozcoPuntoRecarga and necesitoRecargar( sensores ) and objetivo == DESTINO ) {
			destinoPausado = destino;
...
bool ComportamientoJugador::destinoMuchoMasCerca( Sensores sensores ) {
	...
	return casillasADestino < casillasARecarga;
...
/*
	if( sensores.bateria <= accionesRestantes )
		return sensores.bateria <= 1200;
	else
		return true;

	if( accionesRestantes >= sensores.bateria )
		return sensores.bateria >= 1200;
	else
		return true;*/
